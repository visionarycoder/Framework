...
namespace VisionaryCoder.Framework.Proxy.Interceptors.Caching.Interceptors;
    ...
    : IOrderedProxyInterceptor
{
    ...
    public async Task<ProxyResponse<T>> InvokeAsync<T>(ProxyContext context, ProxyDelegate<T> next, CancellationToken cancellationToken = default)
    {
        ...
        {
            TimeSpan expiration = policyProvider.GetExpiration(context) ?? cachePolicy.Duration;
