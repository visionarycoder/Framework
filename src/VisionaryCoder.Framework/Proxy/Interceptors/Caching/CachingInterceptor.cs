...
public sealed class CachingInterceptor : IOrderedProxyInterceptor
{
    ...
    public async Task<ProxyResponse<T>> InvokeAsync<T>(ProxyContext context, ProxyDelegate<T> next, CancellationToken cancellationToken = default)
    {
        ...
        if (cache.TryGetValue(cacheKey, out object? cachedResponse) && cachedResponse is ProxyResponse<T> cached)
        {
        ...
        if (proxyResponse.IsSuccess)
        {
            ...
            cache.Set(cacheKey, proxyResponse, cacheEntryOptions);
            logger.LogDebug("Cached successful proxyResponse for operation '{OperationName}' with key '{CacheKey}' for {Duration}. Correlation ID: '{CorrelationId}'",
