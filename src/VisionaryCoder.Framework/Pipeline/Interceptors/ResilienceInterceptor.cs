...
{
    private readonly AsyncPolicy policy = policy;
    ...
    public static AsyncPolicy DefaultPolicy() =>
        Policy.WrapAsync(Policy.Handle<Exception>().CircuitBreakerAsync(5, TimeSpan.FromSeconds(30)), Policy.Handle<HttpRequestException>().WaitAndRetryAsync(
            [TimeSpan.FromMilliseconds(100), TimeSpan.FromMilliseconds(300), TimeSpan.FromMilliseconds(1000)])
        );
}
