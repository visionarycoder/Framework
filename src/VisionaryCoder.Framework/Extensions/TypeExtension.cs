...
public static class TypeExtension
{
    ...
    /// <returns>The integer value, or null if conversion fails.</returns>
        public static int? AsIntegerOrNull<T>(this T? value)
        {
                ...
                return (value) switch
                {
                        ...
                        float floatValue => floatValue >= int.MinValue && floatValue <= int.MaxValue ? (int)floatValue : (int?)null,
                        uint uintValue => uintValue <= int.MaxValue ? (int)uintValue : (int?)null,
                        _ => null
                };
        }
    ...
    /// <returns>The long value, or null if conversion fails.</returns>
        public static long? AsLongOrNull<T>(this T? value)
        {
                ...
                return (value) switch
                {
                        ...
                        ulong ulongValue => ulongValue <= long.MaxValue ? (long)ulongValue : (long?)null,
                        _ => null
                };
        }
    ...
    /// <returns>The char value, or null if conversion fails.</returns>
        public static char? AsCharOrNull<T>(this T? value)
        {
                ...
                {
                        char charValue => charValue,
                        string stringValue => stringValue.Length > 0 ? stringValue[0] : (char?)null,
                        int intValue => intValue >= char.MinValue && intValue <= char.MaxValue ? (char)intValue : (char?)null,
                        _ => null
                };
        }
