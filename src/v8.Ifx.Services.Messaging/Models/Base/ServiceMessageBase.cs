using vc.Ifx.Services.Messaging.Contract;

namespace vc.Ifx.Services.Messaging.Models.Base;

/// <summary>
///     Abstract base of all service messages, both requests and responses.  Contains common properties to track message correlation
///     and timing throughout the system.
/// </summary>
public abstract class ServiceMessageBase : IServiceMessage
{
    /// <summary>
    ///     Unique Id of the message.  Every request and response has a different Id.
    /// </summary>
    public Guid MessageId { get; set; }

    /// <summary>
    ///     Correlates all messages that help implement a Manager request back to the originating Manager request.
    ///     See ServiceMessageFactory.CreateFrom() for details, and the remarks on that method for usage guidelines.
    /// </summary>
    public Guid CorrelationId { get; set; }

    /// <summary>
    ///     UTC timestamp of when the message was created.
    /// </summary>
    public DateTime TimestampUtc { get; set; }

    /// <summary>
    /// Gets or sets the type of the message (e.g., Request, Response, Fault).
    /// </summary>
    public string MessageType { get; set; } = "Undefined";

    /// <summary>
    /// Gets or sets the name of the source system from which the data originates.
    /// </summary>
    public string SourceSystem { get; set; } = "Undefined";

    // Equality and comparison logic moved to extension methods.
}


