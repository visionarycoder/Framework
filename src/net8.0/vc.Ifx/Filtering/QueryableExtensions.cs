...
public static class QueryableExtensions
{
    ...
    /// <summary>
    ///     Applies ordering to the query based on the specified property name and sortDirection.
    /// </summary>
    /// <typeparam name="T">The type of the entity being queried.</typeparam>
    /// <param name="query">The query to apply ordering to.</param>
    /// <param name="propertyName">The name of the property to order by.</param>
    /// <param name="sortDirection">The sortDirection of the ordering.</param>
    /// <param name="thenBy">Indicates whether this is a secondary ordering.</param>
    /// <returns>The ordered query.</returns>
    private static IQueryable<T> OrderBy<T>(IQueryable<T> query, string propertyName, OrderByCollection.SortDirection sortDirection) where T 
        : class
    {
        ...
        var methodName = sortDirection == OrderByCollection.SortDirection.Descending ? "OrderByDescending" : "OrderBy";
    ...
    /// <param name="thenBy">Indicates whether this is a secondary ordering.</param>
    /// <returns>The ordered query.</returns>
    private static IQueryable<T> ThenBy<T>(IQueryable<T> query, string propertyName, OrderByCollection.SortDirection sortDirection) where T 
        : class
    {
        var parameter = Expression.Parameter(typeof(T), "e");
        var property = Expression.Property(parameter, propertyName);
        ...
        var methodName = sortDirection == OrderByCollection.SortDirection.Descending ? "ThenByDescending" : "ThenBy";
        var resultExpression = Expression.Call(typeof(Queryable), methodName, [typeof(T), property.Type], query.Expression, Expression.Quote(lambda));
    ...
    private static Expression<Func<T, bool>> BuildPredicate<T>(FilterCriteria.Criterion criterion) where T : class
    {
        var parameter = Expression.Parameter(typeof(T), "e");
