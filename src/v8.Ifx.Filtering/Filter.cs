...
public class Filter : IPagination
{
    ...
    private readonly List<OrderByClause> orderByCollection = new();
    public IReadOnlyList<OrderByClause> OrderBy => orderByCollection;
    ...
    public void AddOrderByClause(OrderByClause item)
    {
        if (string.IsNullOrWhiteSpace(item.PropertyName))
        {
            Debug.WriteLine($"The input orderByClause has an invalid PropertyName: '{item.PropertyName}'");
    ...
    public void AddOrderByClauses(params OrderByClause[] items)
    {
    ...
    public record OrderByClause(string PropertyName, SortDirection SortDirection = SortDirection.Undefined);
