...
$files = Get-ChildItem -Path "c:\Dev\GitHub\VisionaryCoder\vc\tests" -Recurse -Filter "*.cs"

foreach ($file in $files) {
    # Get the relative path from tests folder
    $relativePath = $file.FullName.Substring((Resolve-Path "c:\Dev\GitHub\VisionaryCoder\vc\tests").Path.Length + 1)
    ...
    if ($content -match "^\s*namespace\s+([^;\s]+)") {
        ...
        if ($currentNamespace -ne $expectedNamespace -and $expectedNamespace -ne "") {
            Write-Host "Test File: $($file.FullName)"
