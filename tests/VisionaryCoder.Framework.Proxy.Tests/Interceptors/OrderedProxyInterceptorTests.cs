...
namespace VisionaryCoder.Framework.Proxy.Tests.Interceptors;
...
public class OrderedProxyInterceptorTests
{
    ...
    public async Task InvokeAsync_ShouldDelegateToInnerInterceptor()
    {
        ...
        var expectedResponse = Response<string>.Success("test data");
        ProxyDelegate<string> nextDelegate = (ctx, ct) =>
            Task.FromResult(Response<string>.Success("next"));
        ...
        // Act
        var result = await interceptor.InvokeAsync(context, nextDelegate, CancellationToken.None);
    ...
    public async Task InvokeAsync_ShouldPassThroughCancellationToken()
    {
        ...
        mockInner.Setup(i => i.InvokeAsync(
            ...
            .ReturnsAsync(Response<int>.Success(42));
